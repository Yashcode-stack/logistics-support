...
let ServiceabilityService = class ServiceabilityService {
    ...
    constructor(partnerLocationService) {
        this.partnerLocationService = partnerLocationService;
    }
    async checkServiceability(body) {
        const { origin, destination } = body;
        const originDelieveryPartnerToPincodesData = await this.partnerLocationService.getAll({
            pincode: origin,
        }, {}, 0, 100);
        const destinationDelieveryPartnerToPincodesData = await this.partnerLocationService.getAll({
            pincode: destination,
        }, {}, 0, 100);
        const uniqueDeliveryPartners = new Set(originDelieveryPartnerToPincodesData?.map((item) => item?.deliveryPartnerId));
    }
};
